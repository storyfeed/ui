...
class File implements FeedDetail
{
    ...
    /**
     * `Storyfeed/File` — the VOCABULARY'S name, not this package's.
     *
     * A detail outlives whichever library defined it ({@see FeedDetail}), so
     * the name must not contain the library: `storyfeed-ui/file` would repeat
     * the `storyfeed-filament/file` fork one package over. The name is a pure
